...
export function StMiddleSectionFive() {
  return (
    <section className="w-full bg-[#F2F3F5] px-4 pt-10 pb-12 dark:bg-[#0A0A0A] sm:px-6 md:px-8 md:pt-12 md:pb-16">
      <div className="mx-auto w-full max-w-[1180px]">
        <h2 className="mb-6 text-center text-[28px] font-semibold leading-tight text-[#3B7AB6] dark:text-white sm:text-[34px]">
          FAQ
        </h2>
        ...
        <Accordion type="single" collapsible defaultValue="item-1" className="space-y-4">
          {FAQ_ITEMS.map((item) => (
            <AccordionItem key={item.id} value={item.id} className="border-0">
              <AccordionTrigger className="group rounded-[10px] bg-[#0F0D67] px-4 py-5 text-base font-semibold text-white no-underline hover:no-underline dark:bg-[#172036] [&>svg:last-child]:hidden sm:px-6">
              ...
              <AccordionContent className="mt-0 border border-[#D9DDE4] bg-white px-4 py-4 text-[16px] text-[#454F62] dark:border-[#1F2937] dark:bg-[#111827] dark:text-[#D1D5DB] sm:px-6">
                {item.id === "item-3" ? (
