...
import { usePostEbookHtoycacgagMutation } from "@/redux/api/ebookApi";
...
export const EbookForm = () => {
  ...
  const [trigger, { data: htoycacgagData, isError, isSuccess, error, isLoading }] =
  usePostEbookHtoycacgagMutation();

  const { data: formsData } = useGetFormsQuery(
    constants.form_type_ids.how_to_optimise_your_customers_a_comprehensive_guide_achieve_10x_growth,
  );
  ...
  useEffect(() => {
    if (htoycacgagData && isSuccess && htoycacgagData?.statusCode) {
      toast.success(htoycacgagData?.response?.message);
      fireConfetti();
      setCaptchaError(null);
      turnstileRef.current?.reset();
      setCaptchaToken(null);
      form.reset();
      const link = htoycacgagData?.response?.link;
    ...
    if (htoycacgagData && isSuccess && !htoycacgagData?.statusCode) {
      toast.error(htoycacgagData?.response?.message || "Something went wrong");
    }
  ...
  }, [isSuccess, htoycacgagData, isError, error, trigger, form]);
      ...
      <div className="">
        <Highlighter padding={10} action="underline" color="#FF9800">
          <h1 className="text-[25px] xl:text-[30px] font-medium leading-[1.2em]">
            HOW TO OPTIMISE YOUR <br /> CUSTOMERS: A Comprehensive <br /> Guide
            Achieve 10X Growth
          </h1>
        </Highlighter>
      </div>
