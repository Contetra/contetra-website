...
import { usePostEbookBiiisMutation } from "@/redux/api/ebookApi";
...
export const EbookForm = () => {
  ...
  useEffect(() => {
    if (biiisData && isSuccess && biiisData?.statusCode) {
      toast.success(biiisData?.response?.message);
      fireConfetti();
      setCaptchaError(null);
      turnstileRef.current?.reset();
      setCaptchaToken(null);
      form.reset();
      const link = biiisData?.response?.link;
    ...
    if (biiisData && isSuccess && !biiisData?.statusCode) {
      toast.error(biiisData?.response?.message || "Something went wrong");
    }
  ...
  }, [isSuccess, biiisData, isError, error, trigger, form, router]);
    ...
    <div className="w-full md:w-[50%] flex justify-center items-start flex-col gap-10">
      <div className="">
        <Highlighter padding={10} action="underline" color="#FF9800">
          <h1 className="text-[25px] xl:text-[30px] font-medium leading-[1.2em]">Business insights into ifrs 16</h1>
        </Highlighter>
      </div>
      <Form {...form}>
        <form
