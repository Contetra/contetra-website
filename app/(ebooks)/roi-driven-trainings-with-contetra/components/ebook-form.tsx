...
import {  usePostEbookRdtwcMutation } from "@/redux/api/ebookApi";
...
export const EbookForm = () => {
  ...
  const [trigger, { data: rdtwcData, isError, isSuccess, error, isLoading }] =
  usePostEbookRdtwcMutation();

  const { data: formsData } = useGetFormsQuery(
    constants.form_type_ids.roi_driven_trainings_with_contetra,
  );
  ...
  useEffect(() => {
    if (rdtwcData && isSuccess && rdtwcData?.statusCode) {
      toast.success(rdtwcData?.response?.message);
      fireConfetti();
      setCaptchaError(null);
      turnstileRef.current?.reset();
      setCaptchaToken(null);
      form.reset();
      const link = rdtwcData?.response?.link;
    ...
    if (rdtwcData && isSuccess && !rdtwcData?.statusCode) {
      toast.error(rdtwcData?.response?.message || "Something went wrong");
    }
  ...
  }, [isSuccess, rdtwcData, isError, error, trigger, form]);
    ...
    <div className="w-full md:w-[50%] flex justify-center items-start flex-col gap-10">
      <div className="">
        <Highlighter padding={10} action="underline" color="#FF9800">
          ...
          Roi-Driven Trainings With <br/> Contetra
          </h1>
        </Highlighter>
      </div>
      <Form {...form}>
