...
import {  usePostEbookEastipateMutation } from "@/redux/api/ebookApi";
...
export const EbookForm = () => {
  ...
  const [trigger, { data: eastipateData, isError, isSuccess, error, isLoading }] =
  usePostEbookEastipateMutation();

  const { data: formsData } = useGetFormsQuery(
    constants.form_type_ids.actionable_strategies_to_improve_profit_after_tax,
  );
  ...
  useEffect(() => {
    if (eastipateData && isSuccess && eastipateData?.statusCode) {
      toast.success(eastipateData?.response?.message);
      fireConfetti();
      setCaptchaError(null);
      turnstileRef.current?.reset();
      setCaptchaToken(null);
      form.reset();
      const link = eastipateData?.response?.link;
    ...
    if (eastipateData && isSuccess && !eastipateData?.statusCode) {
      toast.error(eastipateData?.response?.message || "Something went wrong");
    }
  ...
  }, [isSuccess, eastipateData, isError, error, trigger, form]);
    ...
    <div className="w-full md:w-[50%] flex justify-center items-start flex-col gap-10">
      <div className="">
        <Highlighter padding={10} action="underline" color="#FF9800">
          ...
          8 Actionable Strategies to Improve <br /> Profit After Tax
          </h1>
        </Highlighter>
      </div>
      <Form {...form}>
