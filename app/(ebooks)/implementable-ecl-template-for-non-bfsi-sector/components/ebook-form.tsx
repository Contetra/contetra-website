...
import { usePostEbookIetfnbsMutation } from "@/redux/api/ebookApi";
...
export const EbookForm = () => {
  ...
  useEffect(() => {
    if (ietfnbsData && isSuccess && ietfnbsData?.statusCode) {
      toast.success(ietfnbsData?.response?.message);
      fireConfetti();
      setCaptchaError(null);
      turnstileRef.current?.reset();
      setCaptchaToken(null);
      form.reset();
      const link = ietfnbsData?.response?.link;
    ...
    if (ietfnbsData && isSuccess && !ietfnbsData?.statusCode) {
      toast.error(ietfnbsData?.response?.message || "Something went wrong");
    }
  ...
  }, [isSuccess, ietfnbsData, isError, error, trigger, form, router]);
      ...
      <div className="">
        <Highlighter padding={10} action="underline" color="#FF9800">
          <h1 className="hidden xl:block text-[25px] xl:text-[30px] font-medium leading-[1.2em]">
            Implementable ECL template <br/> for non-BFSI sector
          </h1>
