...
import {  usePostEbookBgcMutation } from "@/redux/api/ebookApi";
...
export const EbookForm = () => {
  ...
  const [trigger, { data: bgcData, isError, isSuccess, error, isLoading }] =
  usePostEbookBgcMutation();

  const { data: formsData } = useGetFormsQuery(
    constants.form_type_ids.business_growth_compliance,
  );
  ...
  useEffect(() => {
    if (bgcData && isSuccess && bgcData?.statusCode) {
      toast.success(bgcData?.response?.message);
      fireConfetti();
      setCaptchaError(null);
      turnstileRef.current?.reset();
      setCaptchaToken(null);
      form.reset();
      const link = bgcData?.response?.link;
    ...
    if (bgcData && isSuccess && !bgcData?.statusCode) {
      toast.error(bgcData?.response?.message || "Something went wrong");
    }
  ...
  }, [isSuccess, bgcData, isError, error, trigger, form]);
