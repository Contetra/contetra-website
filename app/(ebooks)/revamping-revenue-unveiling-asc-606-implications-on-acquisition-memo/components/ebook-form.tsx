...
import { usePostEbookRruasioamMutation } from "@/redux/api/ebookApi";
...
export const EbookForm = () => {
  ...
  const [trigger, { data: rruasioamData, isError, isSuccess, error, isLoading }] =
  usePostEbookRruasioamMutation();

  const { data: formsData } = useGetFormsQuery(
    constants.form_type_ids.revamping_revenue_unveiling_asc_606_implications_on_acquisition_memo,
  );
  ...
  useEffect(() => {
    if (rruasioamData && isSuccess && rruasioamData?.statusCode) {
      toast.success(rruasioamData?.response?.message);
      fireConfetti();
      setCaptchaError(null);
      turnstileRef.current?.reset();
      setCaptchaToken(null);
      form.reset();
      const link = rruasioamData?.response?.link;
    ...
    if (rruasioamData && isSuccess && !rruasioamData?.statusCode) {
      toast.error(rruasioamData?.response?.message || "Something went wrong");
    }
  ...
  }, [isSuccess, rruasioamData, isError, error, trigger, form]);
    ...
    <div className="w-full md:w-[50%] flex justify-center items-start flex-col gap-10">
      <div className="">
        <Highlighter padding={10} action="underline" color="#FF9800">
          ...
          Revamping Revenue unveiling ASC 606<br /> Implications on Acquisition memo
          </h1>
        </Highlighter>
      </div>
      <Form {...form}>
