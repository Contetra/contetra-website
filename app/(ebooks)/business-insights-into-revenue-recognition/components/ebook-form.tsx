...
import {  usePostEbookBiirrMutation } from "@/redux/api/ebookApi";
...
export const EbookForm = () => {
  ...
  const [trigger, { data: biirrData, isError, isSuccess, error, isLoading }] =
  usePostEbookBiirrMutation();

  const { data: formsData } = useGetFormsQuery(
    constants.form_type_ids.business_insights_into_revenue_recognition,
  );
  ...
  useEffect(() => {
    if (biirrData && isSuccess && biirrData?.statusCode) {
      toast.success(biirrData?.response?.message);
      fireConfetti();
      setCaptchaError(null);
      turnstileRef.current?.reset();
      setCaptchaToken(null);
      form.reset();
      const link = biirrData?.response?.link;
    ...
    if (biirrData && isSuccess && !biirrData?.statusCode) {
      toast.error(biirrData?.response?.message || "Something went wrong");
    }
  ...
  }, [isSuccess, biirrData, isError, error, trigger, form]);
    ...
    <div className="w-full md:w-[50%] flex justify-center items-start flex-col gap-10">
      <div className="">
        <Highlighter padding={10} action="underline" color="#FF9800">
          ...
          Business Insights into Revenue <br/> Recognition
          </h1>
        </Highlighter>
      </div>
      <Form {...form}>
