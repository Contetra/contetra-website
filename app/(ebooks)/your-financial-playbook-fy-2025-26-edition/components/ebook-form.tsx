...
import {  usePostEbookYfpfeMutation } from "@/redux/api/ebookApi";
...
export const EbookForm = () => {
  ...
  const [trigger, { data: yfpfeData, isError, isSuccess, error, isLoading }] =
  usePostEbookYfpfeMutation();

  const { data: formsData } = useGetFormsQuery(
    constants.form_type_ids.your_financial_playbook_fy_2025_26_edition,
  );
  ...
  useEffect(() => {
    if (yfpfeData && isSuccess && yfpfeData?.statusCode) {
      toast.success(yfpfeData?.response?.message);
      fireConfetti();
      setCaptchaError(null);
      turnstileRef.current?.reset();
      setCaptchaToken(null);
      form.reset();
      const link = yfpfeData?.response?.link;
    ...
    if (yfpfeData && isSuccess && !yfpfeData?.statusCode) {
      toast.error(yfpfeData?.response?.message || "Something went wrong");
    }
  ...
  }, [isSuccess, yfpfeData, isError, error, trigger, form]);
    ...
    <div className="w-full md:w-[50%] flex justify-center items-start flex-col gap-10">
      <div className="">
        <Highlighter padding={10} action="underline" color="#FF9800">
          ...
          Your Financial Playbook fy <br />2025-26 edition
          </h1>
        </Highlighter>
      </div>
      <Form {...form}>
