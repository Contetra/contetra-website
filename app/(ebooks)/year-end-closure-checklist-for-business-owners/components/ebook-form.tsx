...
import {  usePostEbookYeccfboMutation } from "@/redux/api/ebookApi";
...
export const EbookForm = () => {
  ...
  const [trigger, { data: yeccfboData, isError, isSuccess, error, isLoading }] =
  usePostEbookYeccfboMutation();

  const { data: formsData } = useGetFormsQuery(
    constants.form_type_ids.year_end_closure_checklist_for_business_owners,
  );
  ...
  useEffect(() => {
    if (yeccfboData && isSuccess && yeccfboData?.statusCode) {
      toast.success(yeccfboData?.response?.message);
      fireConfetti();
      setCaptchaError(null);
      turnstileRef.current?.reset();
      setCaptchaToken(null);
      form.reset();
      const link = yeccfboData?.response?.link;
    ...
    if (yeccfboData && isSuccess && !yeccfboData?.statusCode) {
      toast.error(yeccfboData?.response?.message || "Something went wrong");
    }
  ...
  }, [isSuccess, yeccfboData, isError, error, trigger, form]);
