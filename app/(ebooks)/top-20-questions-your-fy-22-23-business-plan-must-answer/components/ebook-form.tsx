...
import {  usePostEbookTtqyfbpaMutation } from "@/redux/api/ebookApi";
...
export const EbookForm = () => {
  ...
  useEffect(() => {
    if (ttqyfbpaData && isSuccess && ttqyfbpaData?.statusCode) {
      toast.success(ttqyfbpaData?.response?.message);
      fireConfetti();
      setCaptchaError(null);
    ...
    if (ttqyfbpaData && isSuccess && !ttqyfbpaData?.statusCode) {
      toast.error(ttqyfbpaData?.response?.message || "Something went wrong");
    }
  ...
  }, [isSuccess, ttqyfbpaData, isError, error, trigger, form, router]);
