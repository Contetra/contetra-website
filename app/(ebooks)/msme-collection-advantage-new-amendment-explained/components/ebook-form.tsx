...
import { usePostEbookMcanaeMutation } from "@/redux/api/ebookApi";
...
export const EbookForm = () => {
  ...
  const [trigger, { data: mcanaeData, isError, isSuccess, error, isLoading }] =
  usePostEbookMcanaeMutation();

  const { data: formsData } = useGetFormsQuery(
    constants.form_type_ids.msme_collection_advantage_new_amendment_explained,
  );
  ...
  useEffect(() => {
    if (mcanaeData && isSuccess && mcanaeData?.statusCode) {
      toast.success(mcanaeData?.response?.message);
      fireConfetti();
      setCaptchaError(null);
      turnstileRef.current?.reset();
      setCaptchaToken(null);
      form.reset();
      const link = mcanaeData?.response?.link;
    ...
    if (mcanaeData && isSuccess && !mcanaeData?.statusCode) {
      toast.error(mcanaeData?.response?.message || "Something went wrong");
    }
  ...
  }, [isSuccess, mcanaeData, isError, error, trigger, form]);
    ...
    <div className="w-full md:w-[50%] flex justify-center items-start flex-col gap-10">
      <div className="">
        <Highlighter padding={10} action="underline" color="#FF9800">
          ...
          MSME Collection Advantage: <br />New Amendment Explained!
          </h1>
        </Highlighter>
      </div>
      <Form {...form}>
