...
import {  usePostEbookPccMutation } from "@/redux/api/ebookApi";
...
export const EbookForm = () => {
  ...
  const [trigger, { data: pccData, isError, isSuccess, error, isLoading }] =
  usePostEbookPccMutation();

  const { data: formsData } = useGetFormsQuery(
    constants.form_type_ids.period_closure_checklist,
  );
  ...
  useEffect(() => {
    if (pccData && isSuccess && pccData?.statusCode) {
      toast.success(pccData?.response?.message);
      fireConfetti();
      setCaptchaError(null);
      turnstileRef.current?.reset();
      setCaptchaToken(null);
      form.reset();
      const link = pccData?.response?.link;
    ...
    if (pccData && isSuccess && !pccData?.statusCode) {
      toast.error(pccData?.response?.message || "Something went wrong");
    }
  ...
  }, [isSuccess, pccData, isError, error, trigger, form]);
