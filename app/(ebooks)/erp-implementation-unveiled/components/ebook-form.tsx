...
import { usePostEbookEiuMutation } from "@/redux/api/ebookApi";
...
export const EbookForm = () => {
  ...
  const [trigger, { data: eiuData, isError, isSuccess, error, isLoading }] =
    usePostEbookEiuMutation();

  const { data: formsData } = useGetFormsQuery(
    constants.form_type_ids.erp_implementation_unveiled,
  );
  ...
  useEffect(() => {
    if (eiuData && isSuccess && eiuData?.statusCode) {
      toast.success(eiuData?.response?.message);
      fireConfetti();
      setCaptchaError(null);
      turnstileRef.current?.reset();
      setCaptchaToken(null);
      form.reset();
      const link = eiuData?.response?.link;
    ...
    if (eiuData && isSuccess && !eiuData?.statusCode) {
      toast.error(eiuData?.response?.message || "Something went wrong");
    }
  ...
  }, [isSuccess, eiuData, isError, error, trigger, form]);
      ...
      <div className="">
        <Highlighter padding={10} action="underline" color="#FF9800">
          <h1 className="text-[25px] xl:text-[30px] font-medium leading-[1.2em]">
            ...
            <br /> Your Guide to the Why, What & <br /> Whom - Download Now!
          </h1>
        </Highlighter>
      </div>
      <Form {...form}>
